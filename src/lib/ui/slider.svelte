...
<script lang="ts">
  ...
  const { slider, range, thumb, value: valueStore, options } = createSlider({
...
<span melt={$slider} class={cn("group relative flex touch-none select-none items-center overflow-hidden data-[orientation=vertical]:flex-col", orientation === 'vertical' ? 'h-full min-h-[4rem] w-fit' : 'w-full', className)}>
  ...
  {#each { length: $valueStore.length } as _, idx}
    <span
      melt={$thumb()}
      class={cn("block rounded-full focus-visible:outline-none bg-primary shadow group-disabled:pointer-events-none group-disabled:opacity-50 dark:shadow-[inset_0_1px_0_#ffffff2f] transition", orientation === 'vertical' ? 'w-5 h-1 mb-1' : 'h-5 sm:h-3 w-1 ml-2')}
    />
  {/each}
</span>