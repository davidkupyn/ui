...
<span {...$slider} class={cn("group relative flex w-full touch-none select-none items-center", className)}>
  <span class="relative h-1.5 w-full grow overflow-hidden rounded-full bg-accent-500/20">
    <span {...$range} class="absolute h-full bg-accent" />
  </span>
  ...
  {#each { length: $valueStore.length } as _}
    <span
      {...$thumb()}
      use:thumb
      class="block h-4 w-4 rounded-full focus-visible:outline-none border border-accent-500/50 bg-background shadow transition-colors focus-visible:ring-1 focus-visible:ring-accent group-disabled:pointer-events-none group-disabled:opacity-50"
    />
  {/each}
</span>