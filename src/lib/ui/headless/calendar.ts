import type { DelegateEvent } from './helpers/events';
import { delegateEventListeners } from './helpers/events';
...
export const createCalendar = (config?: CalendarConfig) => {
	...
	const goToDate = (date: Date) => {
		...
		tick().then(() => {
			...
			rootNode
				?.querySelector<HTMLElement>(`[data-calendar-day="${date.toISOString().slice(0, 10)}"]`)
				?.focus();
		});
	};
	...
	const onDayKeyDown = (event: DelegateEvent<KeyboardEvent>) => {
		...
		if (event.key === 'ArrowLeft') {
			event.preventDefault();
			goToDate(new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate() - 1)));
		}
		...
		if (event.key === 'ArrowUp') {
			event.preventDefault();
			goToDate(new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate() - 7)));
		}
		...
		if (event.key === 'PageUp') {
			event.preventDefault();
			goToDate(new Date(Date.UTC(date.getFullYear(), date.getMonth() - 1, date.getDate())));
		}
	...
	const days: Readable<CalendarDay[]> = derived(
		...
		([month, year, selected, disabled, startWeekday]) => {
			...
			for (let i = previousMonthDaysToAdd; i > 0; i--) {
				data.push({
					date: new Date(Date.UTC(year, month - 1, lastDayOfPreviousMonth.getDate() - i + 1)),
					isOutOfMonth: true
				});
			}
